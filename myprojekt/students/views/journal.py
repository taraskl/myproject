...
class JournalView(TemplateView):
	...
	def get_context_data(self, **kwargs):
		...
		context = super(JournalView, self).get_context_data(**kwargs)
											...
											#check if we need to display some sprcific mouth
		...
		update_url = reverse('journal')		# url to update student presence, for form post
		
											# go over all students and collect data about presence # during selected month

		students = []
		for student in queryset:			# try to get journal object by month selected # month and current student
			try:
				journal = MonthJournal.objects.get(student=student, date=month)
			except Exception:
				journal = None
		
			days = []						# fill in days presence list for current student
			...
			students.append({				# prepare metadata for current student
